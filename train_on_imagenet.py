...
# 在命令行不调用no-prefetcher，则该参数为False
parser.add_argument('--no-prefetcher', action='store_true', default=False,
                    help='disable fast prefetcher')
parser.add_argument('--output', default='', type=str, metavar='PATH',
                    help='path to output folder (default: none, current dir)')
parser.add_argument('--eval-metric', default='top1', type=str, metavar='EVAL_METRIC',
                    help='Best metric (default: "top1"')  # 结果的评定指标
...
parser.add_argument('--transfer-model', type=str, default="./checkpoint/backbone.pth",
...
def main():
    ...
    model = create_model(
        ...
        bn_eps=args.bn_eps,
        checkpoint_path=args.initial_checkpoint,  # 用这个权重初始化模型，默认为空
    ...
    if args.transfer_learning:
        ...
        load_for_transfer_learning(model, args.transfer_model, use_ema=False, strict=False, num_classes=args.num_classes)

    # 利用timm中的resolve_data_config来设置图片的通道数，尺寸，插值方法，正则化的mean和std，以及裁剪的比例
    data_config = resolve_data_config(vars(args), model=model, verbose=args.local_rank == 0)

    num_aug_splits = 0
    # args.aug_splits,增强分割数，默认为0
    if args.aug_splits > 0:
        assert args.aug_splits > 1, 'A split of 1 makes no sense'
        num_aug_splits = args.aug_splits

    # args.split_bn:每次增强拆分启用单独的BN层，默认是False
    if args.split_bn:
    ...
    if args.num_gpu > 1:
        ...
        # pytorch1.6之前使用apex，之后使用amp，所以走这个分支
        model.cuda()
    ...
    if args.transfer_learning:
        ...
        parameters = [{'params': model.sample.parameters()},
                      {'params': model.tokens_to_token.parameters(),'lr': args.transfer_ratio * args.lr},
                      {'params': model.blocks.parameters(), 'lr': args.transfer_ratio * args.lr},
                      {'params': model.head.parameters(),'lr': args.transfer_ratio * args.lr}
        ]
    else:
    ...
    if use_amp == 'apex':
        ...
        loss_scaler = NativeScaler()  # NativeScaler()就是pytorch的GradScaler()
        if args.local_rank == 0:
    ...
    if args.model_ema:
        model_ema = ModelEma(
            ...
            # args.model_ema_force_cpu default is False
            device='cpu' if args.model_ema_force_cpu else '',
            resume=args.resume)

    if args.distributed:
        if args.sync_bn: # 默认是False
    ...
    lr_scheduler, num_epochs = create_scheduler(args, optimizer)  # 在create_scheduler中实现了warmup的设置
    # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, eta_min=args.min_lr, T_max=args.epochs)
    start_epoch = 0
    ...
    if mixup_active:
        ...
        mixup_args = dict(
            ...
            # args.smoothing default is 0.1
            label_smoothing=args.smoothing, num_classes=args.num_classes)
        if args.prefetcher:
            # args.prefetcher为True，走这个分支
            assert not num_aug_splits  # collate conflict (need to support deinterleaving in collate mixup)
            collate_fn = FastCollateMixup(**mixup_args)  # 对每个批次的数据进行cutmix，mixup，label smooth
        else:
            mixup_fn = Mixup(**mixup_args)

    if num_aug_splits > 1:  # 默认为0
        dataset_train = AugMixDataset(dataset_train, num_splits=num_aug_splits)

    train_interpolation = args.train_interpolation
    # args.no_aug:取消所有训练时的数据增强，默认是False
    if args.no_aug or not train_interpolation:
        train_interpolation = data_config['interpolation']

    # 迁移到压缩学习上的时候训练集只使用RandomResizeCrop和随机水平翻转，对应args.scale,args.ratio,args.hflip
    ...
    loader_eval = create_loader(
        ...
        pin_memory=args.pin_mem,
    )  # 验证集不使用数据增强，只进行了resize和中心裁剪

    if args.jsd:
        # args.jsd：启用启用Jensen Shannon发散，和aug_split一起使用
        assert num_aug_splits > 1  # JSD only valid with aug splits set
        train_loss_fn = JsdCrossEntropy(num_splits=num_aug_splits, smoothing=args.smoothing).cuda()
    elif mixup_active:
        # mixup_active默认为True，所以训练时使用的是SoftTargetCrossEntropy
    ...
    if args.local_rank == 0:
        ...
        # './output/train/yymmdd-hhmmss-args.model-img_size'
        output_dir = get_outdir(output_base, 'train', exp_name)
        decreasing = True if eval_metric == 'loss' else False  # default:top1
        # CheckpointSaver中的max_history=10，指定只存储近10轮的权重
        saver = CheckpointSaver(
            model=model, optimizer=optimizer, args=args, model_ema=model_ema, amp_scaler=loss_scaler,
            checkpoint_dir=output_dir, recovery_dir=output_dir, max_history=4, decreasing=decreasing)
    ...
    try:  # train the model
        for epoch in range(start_epoch, num_epochs):
            ...
            # args.dist_bn default :empty
            # args.dist_bn：在每个epoch之后在节点之间分发BatchNorm的信息
...
def train_epoch(
    ...
    # args.mixup_off_epoch,在当前epoch后关闭mixup，默认为0，表示禁用该功能
    if args.mixup_off_epoch and epoch >= args.mixup_off_epoch:
        # args.prefetcher is True
    ...
    num_updates = epoch * len(loader)  # 当前epoch开始时已经更新过的次数
    for batch_idx, (input, target) in enumerate(loader):
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)  # update()函数记录了当前这次迭代的数据加载时间，和到目前为止的平均加载时间

        # 如果args.prefetcher是True，则mixup_fn是None，args.prefetcher默认是True
        if not args.prefetcher:
            # 疑问：not args.prefetcher是False，那么input和target怎么放到CUDA上的？
            # 答：在数据预提取的过程中input和target已经放到了CUDA上
            input, target = input.cuda(), target.cuda()
            if mixup_fn is not None:
                input, target = mixup_fn(input, target)
        if args.channels_last:
            # args.channels_last 默认是False
            input = input.contiguous(memory_format=torch.channels_last)

        with amp_autocast():
            output = model(input)
            # 训练时的loss要反向传播，所以要放进with amp_autocast()
            loss = loss_fn(output, target)  # 训练过程使用的是SoftTargetCrossEntropy
        if not args.distributed:
            losses_m.update(loss.item(), input.size(0))

        optimizer.zero_grad()
        if loss_scaler is not None:
            # loss_scalar默认是pytorch的GradScaler()
            loss_scaler(
            ...
            optimizer.step()  # 对model的参数进行更新

        torch.cuda.synchronize()  # 等待所有进程上的model更新完毕
        if model_ema is not None:
            model_ema.update(model)  # 调用model_ema的update方法，求model当前时刻的指数移动均值
        num_updates += 1

        # 每个epoch训练的过程中会记录下处理完一个batch的时间
        # 然后在输出信息的时候输出通过batch_time_m.avg输出平均每个batch的处理时间
        batch_time_m.update(time.time() - end)

        # args.log_interval default=50
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                # 单机多卡下args.world_size代表GPU的数目
                # 多机多卡下args.world_size代表机器的数目
            ...
            if args.local_rank == 0:
                _logger.info(
                    ...
                    .format(
                        ...
                        # 因为是并行处理，一个batch_time_m中总共处理了input.size(0) * args.world_size张图片
                        rate=input.size(0) * args.world_size / batch_time_m.val,
                        # 例如512张图片用了10s处理，那么512/10等于51.2张/秒
                        # rate_avg指的是平均每秒处理的图片数，即处理速度
                        rate_avg=input.size(0) * args.world_size / batch_time_m.avg,
                        lr=lr,
                        data_time=data_time_m))

                # args.save_images默认是False
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            reduce_factor = args.tta  # args.tta默认是0
            if reduce_factor > 1:
